...
export default function ReviewsCarousel() {
  ...
  const handlePrev = () => {
    setActiveIdx((prev) => (prev === 0 ? GUEST_REVIEWS.length - 1 : prev - 1));
  };

  const handleNext = () => {
    setActiveIdx((prev) => (prev === GUEST_REVIEWS.length - 1 ? 0 : prev + 1));
  };

  const currentReview = GUEST_REVIEWS[activeIdx];
